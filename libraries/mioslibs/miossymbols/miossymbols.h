...
#define MIOSERR_NOENUMSYMBOLS -2
...
MiosSymbol::MiosSymbol(LPCWSTR pdbPath) {
	...
	if (hr == S_OK) {
		...
		SetLastError(MIOSSUCCESS);
	}
	else {
		printf("Failed to create MiosSymbol object. HRESULT: 0x%08X\n", (unsigned)hr);
		SetLastError(MIOSERR_FAILEDTOCREATEINSTANCE);
	}
}
...
int MiosSymbol::GetSymbolAddresses(DWORD* addresses, LPCWSTR* symNames, int symAmount) {
	...
	while (MiosEnumSymbols->Next(1, &MiosCurrentSymbol, &count) == S_OK) {
		...
		MiosCurrentSymbol->Release();
		MiosCurrentSymbol = NULL;
	}
